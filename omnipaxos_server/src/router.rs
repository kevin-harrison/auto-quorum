...
pub struct Router {
    ...
    buffer: VecDeque<NetworkMessage>,
}
...
impl Router {
    ...
    pub async fn send_message(&mut self, node: NodeId, msg: NetworkMessage) -> Result<(), Error> {
        if let Some(connection) = self.nodes.get_mut(&node) {
            ...
            if let NetworkMessage::OmniPaxosMessage(Message::BLE(_)) = msg {
    ...
    async fn add_node(&mut self, node: NodeId) -> Result<(), Error> {
        if let Some(address) = self.node_addresses.get(&node) {
            ...
            framed.send(NetworkMessage::Hello(self.id)).await?;
...
impl Stream for Router {
    type Item = Result<NetworkMessage, Error>;
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        for mut pending in new_pending.into_iter() {
            if let Poll::Ready(val) = Pin::new(&mut pending).poll_next(cx) {
                match val {
                    Some(Ok(NetworkMessage::Hello(id))) => {
                        debug!("Node {} handshake complete", id);
                        self_mut.buffer.push_back(NetworkMessage::Hello(id));
                        self_mut.nodes.insert(id, pending);
                    }
                    Some(Ok(NetworkMessage::ClientMessage(msg))) => {
                        self_mut.buffer.push_back(NetworkMessage::ClientMessage(msg));
                    }
